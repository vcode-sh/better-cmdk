...
const Command = React.forwardRef<HTMLDivElement, CommandProps>(
    (props, forwardedRef) => {
        ...
        const ids = useLazyRef<
            Map<string, { value: string; keywords?: string[] }>
        >(() => new Map()) // id → { value, keywords }
        ...
        // biome-ignore lint/correctness/useExhaustiveDependencies: intentional stable store from cmdk fork
        ...
        // biome-ignore lint/correctness/useExhaustiveDependencies: intentional stable context from cmdk fork
            ...
            groups
                ...
                .forEach((group) => {
                    const element = listInnerRef.current?.querySelector(
                        `${GROUP_SELECTOR}[cmdk-group-id="${group[0]}"]`,
                    )
                    element?.parentElement?.appendChild(element)
                })

            // Push unscored groups (e.g. forceMount groups with no matching items) to the end
            const sortedGroupIds = new Set(groups.map((g) => g[0]))
            listInnerRef.current
                ?.querySelectorAll(`${GROUP_SELECTOR}[cmdk-group-id]`)
                ?.forEach((el) => {
                    const groupId = el.getAttribute("cmdk-group-id")
                    if (groupId && !sortedGroupIds.has(groupId)) {
                        el.parentElement?.appendChild(el)
                    }
                })

            // Reset scroll position after DOM reordering to neutralize Chrome's scroll anchoring
            const scrollContainer = listInnerRef.current?.closest("[cmdk-list]")
        ...
        function updateSelectedByItem(change: 1 | -1) {
            ...
            if (newSelected)
                store.setState(
                    "value",
                    newSelected.getAttribute(VALUE_ATTR) ?? "",
                )
        }
...
const Item = React.forwardRef<HTMLDivElement, ItemProps>(
    (props, forwardedRef) => {
        ...
        const propsRef = useAsRef(props)
        const forceMount =
            propsRef.current?.forceMount ?? groupContext?.forceMount
        ...
        // biome-ignore lint/correctness/useExhaustiveDependencies: intentional deps from cmdk fork
        React.useEffect(() => {
...
const Group = React.forwardRef<HTMLDivElement, GroupProps>(
    (props, forwardedRef) => {
        ...
        // biome-ignore lint/correctness/useExhaustiveDependencies: id is stable within mount from cmdk fork
...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
    (props, forwardedRef) => {
        ...
        // biome-ignore lint/correctness/useExhaustiveDependencies: store.setState is stable from cmdk fork
        React.useEffect(() => {
            if (props.value != null) {
